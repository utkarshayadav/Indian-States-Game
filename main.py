...
data = pandas.read_csv("state_names.csv")
all_states = data.state.to_list()
guessed_states = []

while len(guessed_states) < 28:
    answer_state = screen.textinput(title=f"{len(guessed_states)}/28 States guessed", prompt="Whats's another state's name?")
    ...
    if answer_state.lower() in map(str.lower, all_states):
